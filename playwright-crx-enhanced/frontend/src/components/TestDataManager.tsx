...
const TestDataManager = () => {
  ...
  const loadData = async () => {
    try {
      const suitesStr = localStorage.getItem('test-suites');
      const dataStr = localStorage.getItem('test-data');
      
      if (suitesStr) setTestSuites(JSON.parse(suitesStr));
      if (dataStr) setTestData(JSON.parse(dataStr));
    } catch (error) {
      ...
      localStorage.setItem('test-suites', JSON.stringify(defaultSuites));
      localStorage.setItem('test-data', JSON.stringify(defaultData));
    }
  };

  const saveData = async (suites: TestSuite[], data: TestDataItem[]) => {
    try {
      localStorage.setItem('test-suites', JSON.stringify(suites));
